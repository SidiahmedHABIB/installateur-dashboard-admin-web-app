...
export class RemoteDataService {
  ...
  public postLogin(url: string, email: string, password: string): Observable<any> {
    const grantType = 'password';
    const withRefreshToken = 'true';
    const refreshToken = '';
    const formData = new FormData();
  ...
  public async postRequest(url: string, body: any): Promise<any> {
    ...
    //   });
    return this.httpClient.post<any>(this.BASEURL + url, body).toPromise();
    // } catch (error) {
  ...
  public postEmail(url: string, body: any): Observable<any> {
    return this.httpClient.post<any>(this.BASEURL + url, body, {
      headers: {
